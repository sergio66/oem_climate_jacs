...
%elseif iDoConvolve > 0 & iDoRad == 3 & exitcode == 0
...
if iDoConvolve > 0 & iDoRad == 3 & iDoJac == 101 & exitcode == 0 & exitcodec == 0
  ...
  do_convolve_jac(iInstr,iiBin,1);    %% do the 100 layer T/W/O
  disp(' .... doing jac col convolve ...')    
  do_convolve_jac(iInstr,iiBin,100);  %% do the column jacs
elseif iDoConvolve > 0 & iDoRad == 3 & iDoJac == 100 & exitcodec == 0
  disp(' .... doing rad convolve ...')
  do_convolve(iInstr,iiBin);
  disp(' .... doing jac col convolve ...')    

  do_convolve_jac(iInstr,iiBin,100,iCLD);  %% do the column jacs

end

disp('done with all convolutions')
put_together_results(iInstr,iiBin);
rmer_all(iiBin);
