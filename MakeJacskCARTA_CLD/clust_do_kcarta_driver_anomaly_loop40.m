  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if iColJacOnly < 0
      if iDoConvolve > 0 & iDoRad == 3 & iDoJac == 101 & exitcode == 0 & exitcodec == 0
        ...
        do_convolve(iInstr,iTimeStep,iiBin,iDoRad,kcvers,use_this_rtp);
        disp(' .... doing jac(z) convolve ...')  
        do_convolve_jac(iInstr,iTimeStep,iiBin,1,kcvers,use_this_rtp);    %% do the 100 layer T/W/O
        disp(' .... doing jac col convolve ...')    
        do_convolve_jac(iInstr,iTimeStep,iiBin,100,kcvers,use_this_rtp);  %% do the column jacs
      end
    elseif iColJacOnly > 0								     
    ...
    if iColJacOnly == -1
      ...
      rmer_all(iInstr,iTimeStep,iiBin);
    else
      put_together_results_coljac(iInstr,iTimeStep,iiBin);
      cper_coljac(iTimeStep,iiBin);
      rmer_all(iInstr,iTimeStep,iiBin);
    end
...
disp('all done kcarta CLD jac, final check of blowing away files')
lser_and_rmer
