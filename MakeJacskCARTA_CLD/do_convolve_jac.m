function [w,d] = do_convolve_jac(iInstr,iTimeStep,iiBin,iDoJac,kcvers,use_this_rtp);
...
if iDoJac == 1
  if kcvers >= 118
    [dall,w,natmos, numlay, ngases] = readkcjac([outdirtemp '/jac.dat' num2str(iiBin)]);
  elseif kcvers == 116 | kcvers == 114 | kcvers == 112
    [dall,w,natmos, numlay, ngases] = readkcjacv116([outdirtemp '/jac.dat' num2str(iiBin)]);
  else
    kcvers
    error('oopsy kcvers')  
  end
elseif iDoJac == 100
  if kcvers >= 118
    [dall,w] = readkcBasic([outdirtemp '/jacc.dat' num2str(iiBin) '_COL']);
  elseif kcvers == 116
    [dall,w] = readkcBasicv116([outdirtemp '/jacc.dat' num2str(iiBin) '_COL']);
  else
    kcvers
    error('oopsy kcvers did not run column jacs since this was not in v112')  
  end
  fprintf(1,'pclsam jac : need %s %s \n',[outdirtemp '/radc.dat' num2str(iiBin)],[outdirtemp '/jacc.dat' num2str(iiBin) '_COL'])
  [junk_rad,dall,w] = readkcPCLSAM_coljac([outdirtemp '/radc.dat' num2str(iiBin)],[outdirtemp '/jacc.dat' num2str(iiBin) '_COL']);
end
...
if iInstr == 1
  ...
  if iDoJac == 1
    saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_airs_' num2str(iiBin) '_jac.mat fKc rKc sfile  use_this_rtp '];
  else
    saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_airs_' num2str(iiBin) '_coljac.mat fKc rKc sfile  use_this_rtp '];
  end
  eval(saver)
...
elseif iInstr == 2
  ...
  if iDoJac == 1  
    saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_iasi_' num2str(iiBin) '_jac.mat fiasi rKcIasi_all use_this_rtp'];
  else
    saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_iasi_' num2str(iiBin) '_coljac.mat fiasi rKcIasi_all use_this_rtp'];
  end
  eval(saver)
...
elseif iInstr == 4 | iInstr == 14 | iInstr == 124
  ...
  %stemp = p.stemp;
  if iDoJac == 1    
    saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_crisHI_' num2str(iiBin) '_jac.mat rcris_all fcris use_this_rtp'];
  else
    saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_crisHI_' num2str(iiBin) '_coljac.mat rcris_all fcris use_this_rtp'];
  end
  ...
  if iInstr == 14 | iInstr == 124
    ...
    [fKc,rKc] = convolve_airs(w,dall,clist,sfile);
    if iDoJac == 1      
      saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_AIRS_crisHI_' num2str(iiBin) '_jac.mat rcris_all fcris use_this_rtp'];
      saver = [saver ' fKc rKc sfile'];
    else
      saver = ['save ' outdirtemp '//individual_prof_convolved_kcarta_AIRS_crisHI_' num2str(iiBin) '_coljac.mat rcris_all fcris use_this_rtp'];
      saver = [saver ' fKc rKc sfile'];
    end
  end
...
fprintf(1,'do_convolve_jac.m : iInstr,iiBin,iDoJac = %3i %3i %3i\n',iInstr,iiBin,iDoJac)
