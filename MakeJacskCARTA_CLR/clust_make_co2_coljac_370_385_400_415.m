...
for latbin = 1:40
  ...
  for iipert = 1 : length(iaCO2)
    ...
    kcrad = ['CO2_370_385_400_415/junkCO2_latbin_' num2str(latbin) '_co2pert_' num2str(iipert) '.dat'];
    kcjac = ['CO2_370_385_400_415/junkCO2_latbin_' num2str(latbin) '_co2pert_' num2str(iipert) '.jac_COL'];
...
%% above is jac for 10% change (which starting from eg 370 is 3*dQkc ppmv change ... so what is it for 2.2 ppmv change??)
  qcx = qc * (2.2./(370 * dQkc));
  plot(fc,qcx)
  plot(fc,qcx./(qcx(:,1)*ones(1,length(iaCO2)))); axis([640 2780 0.9 1.25])

  %% this is better way
  iaDelta = ones(length(qcx),1) * 2.2./(iaCO2 * dQkc);
  qcx = qc .* iaDelta;
  ...
  saver = ['save CO2_370_385_400_415/co2_jac_2834_latbin' num2str(latbin) '.mat fc qc qcx iaCO2 comment'];
  eval(saver);
