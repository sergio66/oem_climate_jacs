...
%for ll = 1:40
  ...
  for ii = 1 : length(iaN2O)
    ...
    kcjac = ['N2O_315_340/junkN2O_latbin_' num2str(ll) '_n2opert_' num2str(ii) '.jac_COL'];
...
%% above is jac for 10% change (which starting from eg 1800 ppb is 180 ppmb change ... so what is it for 5 ppmb change??)
  ...
  %% this is better way
  iaDelta = ones(length(qcx),1) * 1.0./((iaN2O) * dQkc);
  qcx = qc .* iaDelta;
  ...
  comment = ['use qcx which is normlaized to deal with 1 ppb for varying (315:5:340) ppb'];
  saver = ['save N2O_315_340/n2o_jac_2834_latbin' num2str(ll) '.mat fc qc qcx iaN2O comment'];
  eval(saver);
