...
pcolor(xtime,plays(1:98),xx); shading interp; 
...
pcolor(xtime,plays(1:98),xx); shading interp; 
  set(gca,'ydir','reverse'); colormap(llsmap4.llsmap4); caxis([-0.2 +0.2]); colorbar
title('T=fracO3(z,time) anomaly,\newline smoothed over one year');

figure(4); plot(xtime,mean(stempanom(tropics,:))); title('Stemp Anom,\newline smoothed over one year');

for ii = 1 : 1
  figure(ii)
  shading flat;
  set(gca,'ydir','reverse'); colormap(usa2); caxis([-1 +1]); colorbar
  axis([2002.75 2018.75 1 1000]); xlabel('time'); ylabel('P(mb)')
  set(gca,'yscale','log'); 
end
for ii = 2 : 2
  figure(ii)
  shading flat;
  set(gca,'ydir','reverse'); colormap(usa2); caxis([-0.2 +0.2]); colorbar
  axis([2002.75 2018.75 1 1000]); xlabel('time'); ylabel('P(mb)')
  set(gca,'yscale','log'); 
end
for ii = 3 : 3
