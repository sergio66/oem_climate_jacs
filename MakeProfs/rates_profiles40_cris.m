...
for ii = iiBin : iiBin
  ...
  [yy,mm,dd,hh] = tai2utcSergio(a.rtime_mean(:,5));
  startT = utc2taiSergio(2012,05,01,0);
  endT   = utc2taiSergio(2019,04,01,0);
  goodtime = find(a.rtime_mean(:,5) >= startT & a.rtime_mean(:,5) <= endT);
  fprintf(1,'need %4i of %4i timesteps for 2012/05 to 2019/04 \n',length(goodtime),length(yy))

  clear h p

  woo = find(isfinite(a.nlevs_mean(goodtime,5)));
  woo = find(isfinite(a.nlevs_mean(goodtime,5)) & isfinite(a.spres_mean(goodtime,5)));
  whos woo
  if length(woo) < length(a.rtime_mean(goodtime,5))
  ...
  p.rtime = a.rtime_mean(woo,5)';
  p.gas_1 = squeeze(a.gas1_mean(woo,5,:))';
  p.gas_3 = squeeze(a.gas3_mean(woo,5,:))';  
  p.ptemp = squeeze(a.ptemp_mean(woo,5,:))';
  p.stemp = a.stemp_mean(woo,5)';
  p.spres = a.spres_mean(woo,5)';
  p.rlat = a.lat_mean(woo,5)';
  p.rlon = a.lon_mean(woo,5)';  
  p.plat = a.lat_mean(woo,5)';
  p.plon = a.lon_mean(woo,5)';  
  p.satzen = a.satzen_mean(woo,5)';
  if iChangeSatZen > 0
  ...
  p.solzen = a.solzen_mean(woo,5)';  
  p.nlevs  = a.nlevs_mean(woo,5)';
  ...
  p.satheight = ones(size(p.stemp)) * 805000;
