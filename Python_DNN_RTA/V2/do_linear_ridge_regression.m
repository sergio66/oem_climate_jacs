...
legend('NN train','NN val','linearregr val'); xlabel('wavenumber (cm^{-1})'); ylabel('RMSE (K)');
...
figure(7); colormap jet;
  yyaxis left;  plot(freq,nanmean(btLinearVal - btTrueVal),'b',freq,nanstd(btLinearVal - btTrueVal,[],1),'r')
    ylabel('bias/std [k]')  
  yyaxis right; plot(freq,nanmean(btLinearVal),'k'); hold on; plot(freq,nanmean(btTrueVal),'color',[1 1 1]*0.6); hold off
