...
if iUseActual_Profs_N_Rads < 0
  ...
  wavenumbers = freq0;
  if iJac == -1
    btRaw       = planckBT(radianceRaw0, wavenumbers);  %% these are rads
  else
    btRaw       = radianceRaw0;   %% these are dBT/dX so already in K/X units
  end
end
...
whos freq0 emissivityRaw0  btRaw radianceRaw0
