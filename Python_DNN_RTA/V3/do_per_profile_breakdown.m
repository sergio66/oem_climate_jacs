...
subplot(143)
scatter(mmw(iFlag(valIdx)), perProfileRMSE, 10, 'filled');
xlabel('mmw (mm)'); ylabel('per-profile RMSE (K)');

subplot(144)
scatter(p.scanang(iFlag(valIdx)), perProfileRMSE, 10, 'filled');
xlabel('scanang'); ylabel('per-profile RMSE (K)');

disp('ret to continue'); pause
%%%%%%%%%%%%%%%%%%%%%%%%%
figure(11);
scatter_coast(p.rlon(iFlag(valIdx)),p.rlat(iFlag(valIdx)),20,perProfileRMSE); title('RMSE [K]')
if iJac < 0
  caxis([0 0.25])
else
  caxis([0 5e-3])
...
figure(11);
if length(intersect(iALOTMP,[2 6 7 8 10])) == 1
  %% ocean
  [nn,nx,ny,nmean,nstd] = myhist2d(surfPressure(valIdx), perProfileRMSE, 900 : 10 : 1200, dBTstats);
  errorbar(900 : 10 : 1200,nmean,nstd); plotaxis2;
    xlabel('spres (mb)'); ylabel('RMSE [K]')
  hold on; plot(900 : 10 : 1200,nmean,'o'); hold off
else
  %% also land
  [nn,nx,ny,nmean,nstd] = myhist2d(surfPressure(valIdx), perProfileRMSE, 500 : 25 : 1200, dBTstats);
  errorbar(500 : 25 : 1200,nmean,nstd); plotaxis2;
    xlabel('spres (mb)'); ylabel('RMSE [K]')
  hold on; plot(500 : 25 : 1200,nmean,'o'); hold off    
end    
disp('ret to continue'); pause
...
[nn,nx,ny,nmean,nstd] = myhist2d(mmw(iFlag(valIdx)), perProfileRMSE, 0:2.5:100, dBTstats);
errorbar(0:2.5:100,nmean,nstd); plotaxis2;
  xlabel('mmw'); ylabel('RMSE [K]')
disp('ret to continue'); pause

[nn,nx,ny,nmean,nstd] = myhist2d(p.scanang(iFlag(valIdx)), perProfileRMSE, 0:2.5:75, dBTstats);
errorbar(0:2.5:75,nmean,nstd); plotaxis2;
  xlabel('scanang'); ylabel('RMSE [K]')
disp('ret to continue'); pause

[nn,nx,ny,nmean,nstd] = myhist2d(p.landfrac(iFlag(valIdx)), perProfileRMSE, 0:0.02:1, dBTstats);
errorbar(0:0.02:1,nmean,nstd); plotaxis2;
  xlabel('landfrac'); ylabel('RMSE [K]')
scatter_coast(p.rlon(iFlag(valIdx)),p.rlat(iFlag(valIdx)),10,p.landfrac(iFlag(valIdx))); title('landfrac')
disp('ret to continue'); pause
...
scatter_coast(p.rlon(iFlag(valIdx)),p.rlat(iFlag(valIdx)),10,emissivityRaw(valIdx,i1231)); title('1231 cm-1 emissivity')
disp('ret to continue'); pause

scatter(surfTemp(valIdx),mmw(iFlag(valIdx)),10,surfPressure(valIdx),'filled'); colorbar;
  xlabel('validation SKT');   ylabel('validation mmw'); title('Colorbar = spres')
disp('ret to continue'); pause

scatter(surfTemp(valIdx),mmw(iFlag(valIdx)),10,perProfileRMSE,'filled'); colorbar;
if iJac < 0
  badThresh = 0.2;
else
  badThresh = 2e-3;
end
xlabel('validation SKT');   ylabel('validation mmw'); title(['Colorbar = RMSE \newline gray crosses are high RMSE > ' num2str(badThresh) ' K']); caxis([0 1]);
hold on; bad = find(perProfileRMSE > badThresh); plot(surfTemp(valIdx(bad)),mmw(iFlag(valIdx(bad))),'x','color',[1 1 1]*0.6); hold off

oceanx = find(p.landfrac(iFlag(valIdx)) == 0);
landx  = find(p.landfrac(iFlag(valIdx)) >  0);
...
plot(p.landfrac(iFlag(valIdx)),emissivityRaw(valIdx,i1231),'.')
  xlabel('landfrac'); ylabel('1231 emissivity'); 
scatter(p.landfrac(iFlag(valIdx)),emissivityRaw(valIdx,i1231),10,perProfileRMSE,'filled'); colorbar
