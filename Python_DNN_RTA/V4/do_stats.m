...
plot(freq,nanmean(btReconVal),'b',freq,nanmean(btTrueVal),'r')
  yyaxis left;  plot(freq,nanmean(btReconVal - btTrueVal),'b',freq,nanstd(btReconVal - btTrueVal,[],1),'r'); plotaxis2;
    ylabel('bias/std [k]')
  yyaxis right; plot(freq,nanmean(btReconVal),'k'); hold on; plot(freq,nanmean(btTrueVal),'color',[1 1 1]*0.6); hold off; plotaxis2;
...
figure(2); clf; colormap jet;
  yyaxis left;  plot(freq,nanmean(btReconTrain - btTrueTrain),'b',freq,nanstd(btReconTrain - btTrueTrain,[],1),'r'); plotaxis2;
    ylabel('bias/std [k]')  
  yyaxis right; plot(freq,nanmean(btReconTrain),'k'); hold on; plot(freq,nanmean(btTrueTrain),'color',[1 1 1]*0.6); hold off; plotaxis2;
    legend('mean(KC-calc)','std(KC-calc)','mean(KC)','location','best')
    ylabel('actual mean [k]'); xlabel('Waveumber cm-1')    
title('NN : Training')    
...
legend('train','val','Signal to Fit'); xlabel('wavenumber (cm^{-1})'); ylabel('RMSE (K)');
