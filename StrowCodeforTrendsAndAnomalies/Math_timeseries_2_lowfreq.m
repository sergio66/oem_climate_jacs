function [y g]=Math_timeseries_2_lowfreq(x,c);
...
for ii=1:(length(c)-2)/2
  y = y + c(2*ii+1)*cos(1/ii*2*pi/365*x) + c(2*ii+2)*sin(1/ii*2*pi/365*x);
end
...
if(nargout>1)
   ...
   g(:,2)=x/365;
   for ii=1:(length(c)-2)/2
      g(:,2*ii+1)=          cos(1/ii*2*pi/365*x);
      g(:,2*ii+2)=          sin(1/ii*2*pi/365*x);
   end
end
