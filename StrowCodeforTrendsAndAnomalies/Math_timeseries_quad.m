function [y g]=Math_timeseries_quad(x,c);
...
y = c(1)       + ...
    c(2)*x/365 + ...
    c(3)*(x/365).^2;
for ii=1:(length(c)-3)/2
  y = y + c(2*ii+2)*cos(ii*2*pi/365*x) + c(2*ii+3)*sin(ii*2*pi/365*x);
end
...
if(nargout>1)
  ...
  g(:,3) = (x/365).^2;
  for ii=1:(length(c)-3)/2
    g(:,2*ii+2)=          cos(ii*2*pi/365*x);
    g(:,2*ii+3)=          sin(ii*2*pi/365*x);
  end
end
