...
% Separate by land/ocean since different emissivities
      ...
      if length(find(kl == 1)) > 10
         ...
         t2616 = NaN(1,length(bt2616));

% First need surface emis to get t1231
         x2 = squeeze(temis(moni,2,ilat,ilon)).*ones(1,length(bt1231));
         x1 = bt1231 - bt1228;
...
% Neural net solution for t1231
         ...
         t2616(kl) = polyval(pfit,y) + bt2616;
      end
...
% Night first  (ignore land vs ocean)
         ...
         tf = isoutlier(t2616(i1) - pc,'grubbs') & t2616(i1) - pc > 2;
...
if exist('fire')
   save('testnew_ilon39_ilat41', 'hot','fire');
else
   save('testnew_ilon39_ilat41','hot');
end
