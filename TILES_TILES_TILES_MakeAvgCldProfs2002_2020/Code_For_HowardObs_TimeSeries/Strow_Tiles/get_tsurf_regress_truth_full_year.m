addpath ~/Matlab
load_fairs

load Data/latB64
lonB2 = -180:5:180;

cd  /asl/rtp/airs/airicrad_emistest/random_stdemis/2018

a = dir('*.rtp');

cd  /asl/rtp/airs/airicrad_emistest/random_stdemis/2019
a2 = dir('*.rtp');

cd ~/Work/Airs/Tiles

l = length(a);
l2 = length(a2);
for i=1:l2
   a(l+i) = a2(i);
end

vars(1).name = 'robs1_1228';
vars(2).name = 'robs1_1231';
vars(3).name = 'rclr_1228';
vars(4).name = 'rclr_1231';
vars(5).name = 'satzen';
vars(6).name = 'solzen';
vars(7).name = 'landfrac';
vars(8).name = 'stemp';
vars(9).name = 'wspeed';
vars(10).name = 'emis_1228';
vars(11).name = 'emis_1231';
vars(12).name = 'rtime';
vars(13).name = 'robs1_2616';
vars(14).name = 'rclr_2616';
vars(15).name = 'emis_2616';

% Either 19 pts (ocean) or 100 pts (land)
% Interpolation inputs
landpts = [38:41];
oceanpts = [10:13];
oceanpts_sw = [14:17];

for rfi = 1:length(a);
   rfi

   [h,ha,p,pa] = rtpread(fullfile(a(rfi).folder,a(rfi).name));

   ch_emis = NaN(3,length(p.rlat));

   freqs = [fairs(1513) fairs(1520)];
   afreqs = repmat(freqs,length(p.rlat),1);

%   k = find(p.nemis == 19);
   k = find(p.landfrac == 0 & p.iudef(4,:) == 68);
   for i=k
      ch_emis(1:2,i) = interp1(p.efreq(oceanpts,i),p.emis(oceanpts,i),freqs,'pchip');
   end

   k = find(p.landfrac == 0 & p.iudef(4,:) == 68);
   for i=k
      ch_emis_sw(:,i) = interp1(p.efreq(oceanpts_sw,i),p.emis(oceanpts_sw,i),fairs(2600),'pchip');
   end

%    % Not really needed
%    k = find(p.landfrac == 1);
%    for i=k
%       ch_emis(1:2,i) = interp1(p.efreq(landpts,i),p.emis(landpts,i),freqs,'pchip');
%    end

% ch_emis(1,:) is the 1228 cm-1 emissivity, ch_emis(2,:) is for 1231 cm-1

% Get which scenes fall into our tiling bins
   grid_ids = bin2d_asl(p.rlat,p.rlon,latB2,lonB2);

   k = find(p.landfrac == 0 & p.iudef(4,:) == 68);

   p.emis_1228 = ch_emis(1,:);
   p.emis_1231 = ch_emis(2,:);
   p.robs1_1228 = p.robs1(3,:);
   p.robs1_1231 = p.robs1(4,:);
   p.rclr_1228 = p.rclr(3,:);
   p.rclr_1231 = p.rclr(4,:);
   p.rclr_2616 = p.rclr(5,:);
   p.robs1_2616 = p.robs1(5,:);
   p.emis_2616 = ch_emis(3,:);
   
   if rfi == 1
% First time
      for i=1:64
         for j = 1:72
            for vi = 1:length(vars)
               pg(i,j).(vars(vi).name) = p.(vars(vi).name)(grid_ids(i,j).k);
            end
         end
      end
   else
% Remaining times
      for i=1:64
         for j = 1:72
            for vi = 1:length(vars)
               pg(i,j).(vars(vi).name) = [pg(i,j).(vars(vi).name) p.(vars(vi).name)(grid_ids(i,j).k)];
            end
         end
      end
   end

% rtp file loop   
end
