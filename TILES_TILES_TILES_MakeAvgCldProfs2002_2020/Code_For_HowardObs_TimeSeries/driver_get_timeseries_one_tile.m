...
t23_35_66 = driver_get_timeseries_one_tile(35,66,23,-1,0);  %% iDorA = -1 ==> asc
% save t23years_lat35_lon66 t23_35_66
...
% function out = get_timeseries_one_tile(latbin,lonbin,iNumYears,iDorA,iLorOorA,findtile)
if nargin == 0
  latbin = 32;
  lonbin = 36;
  iNumYears = 23;
...
for tt = 1 : ttmax
  ...
  if ~exist(fname)
    ...
    if mod(tt,10) == 0
      ...
      figure(1)
      junk = rad2bt(f2645,squeeze(out.quant.mean_rad(tt,:,:))');
      plot(f2645,junk,'b',f2645,rad2bt(f2645,out.min.rad(:,tt)),'g',f2645,rad2bt(f2645,out.max.rad(:,tt)),'r')
      axis([645 1620 200 300])
      title(num2str(tt))      
      junkA = [rad2bt(1231,out.min.rad(1520,tt)) junk(1520,:) rad2bt(1231,out.max.rad(1520,tt))];
      junkB = [junkA; [min(bt1231) qx max(bt1231)]]';
      printarray(junkB,' the averaged data     quantiles')
    
      figure(2)
      pcolor(dbt,1:tt,log10(out.avg.histall')); shading interp; colorbar
      title(num2str(tt))
      pause(0.1)
    end
