...
[P] = nanpolyfit(thetime,smooth(x_anom_WgtA,ceil(23*iNumYearSmooth*less_smooth),'loess'),1); PY = polyval(P,thetime);
...
figure(7); plot(thetime,x_anomA_smooth(indstr_m,:),'--',thetime,x_anomA_smooth(indstr_p,:),thetime,smooth(x_anom_WgtA,ceil(23*iNumYearSmooth*less_smooth),'loess'),'kx-',thetime,PY,'k','linewidth',2); 
if wgtChan2 <= eps
  %% one chan only
