...
while daysX < days_since_2002_E & iFound < 0
  ...
  if exist(fout) & exist(favgRP)
    ...
    scatter_coast(pjunk.rlon,pjunk.rlat,50,rad2bt(1231,pjunk.rcalc(1520,:))); title([num2str(yy0) ':' num2str(JOB,'%03i')]); caxis([200 320]); colorbar
    savercalc.rcalc(iFoundFile,:,:)       = pjunk.rcalc;
    savercalc.rcalc_clear(iFoundFile,:,:) = pjunk.sarta_rclearcalc;
...
save -v7.3 DATA/savercalc.mat savercalc comment
%}
