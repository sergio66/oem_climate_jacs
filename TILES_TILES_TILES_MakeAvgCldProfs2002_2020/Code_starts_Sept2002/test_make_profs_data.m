...
for ii = 1 : length(x)   %% outermost loop : scan latitude
  ...
  rtpwrite('junk.ip.rtp',h,ha,p2,pa);
  klayers_code = '/asl/packages/klayersV205/BinV201/klayers_airs';
  klayerser = ['!' klayers_code ' fin=junk.ip.rtp fout=junk.op.rtp > ugh'];
  eval(klayerser);
  [h2x,ha2x,p2x,pa2x] = rtpread('junk.op.rtp');
  mmw = mmwater_rtp(h2x,p2x);
  p2x.rcalc = p2.rcalc;
  ...
  %{ 
  find(p2.rlat >= 0 & p2.landfrac == 0,1)
  addpath /home/sergio/KCARTA/MATLAB
  [d,w] = readkcstd('testkcarta_1918.dat');
  [fc,qc] = convolve_airs(w,d,1:2378,hdffile);
  plot(fc,rad2bt(fc,qc))

  [hhs,pps] = subset_rtp_allcloudfields(h2x,p2x,[],[],1918);
  rtpwrite('junkprof1918_7chan.op.rtp',hhs,[],pps,[]);
  sartaer = ['!' run_sarta.sartaclear_code ' fin=junkprof1918_7chan.op.rtp fout=junkprof1918_7chan.rp.rtp > ugh'];  
  eval(sartaer);
  [hhs2_7,~,pps2_7,~] = rtpread('junkprof1918_7chan.rp.rtp');

  [hhs,pps] = subset_rtp_allcloudfields(h2x,p2x,[],[],1918);
  hhs = rmfield(hhs,'vchan');
  hhs.ichan = (1:2834)';
  hhs.nchan = 2834;
  rtpwrite('junkprof1918.op.rtp',hhs,[],pps,[]);
  sartaer = ['!' run_sarta.sartaclear_code ' fin=junkprof1918.op.rtp fout=junkprof1918.rp.rtp > ugh'];  
  eval(sartaer);
  [hhs2,~,pps2,~] = rtpread('junkprof1918.rp.rtp');

  plot(hhs2.vchan(1:2378),rad2bt(hhs2.vchan(1:2378),pps2.rcalc(1:2378)),'b.-',hhs2_7.vchan,rad2bt(hhs2_7.vchan,pps2_7.rcalc),'bo',fc,rad2bt(fc,qc),'r')
  %}
  ...
  %figure(4); simplemap(p.rlat,p.rlon,mmw); title('mmw')
  pause(1)
