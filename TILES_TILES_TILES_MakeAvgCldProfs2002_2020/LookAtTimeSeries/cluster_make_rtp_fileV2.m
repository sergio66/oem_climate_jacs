...
disp('running cluster_make_rtp_fileV2')
...
for iXX = 1 : 72
  ...
  fnamecld = ['RTP_PROFSV2/Cld/timestep_lonbin_' num2str(indX,'%02d') '_latbin_' num2str(indY,'%02d') '_JOB_' num2str(JOB,'%04d') '_cld.rtp'];
  ...
  if ~exist(fnameclr) | ~exist(fnamecld) | ~exist(fnameavg) | ~ exist(fnamecold)
    ...
    %% ADD IN SATZEN,SOLZEN,RTIME HERE, DUMMY
    ...
    pall.satzen = mean(ind54.satzen_quantile1231_desc(:,15:16),2)';
    pall.solzen = mean(ind54.solzen_quantile1231_desc(:,15:16),2)';

    pallcold.robs1 = nanmean(ind54.rad_quantile_desc(:,:,1:2),3)';     %% take coldest
    pallcold.satzen = mean(ind54.satzen_quantile1231_desc(:,1:2),2)';
    pallcold.solzen = mean(ind54.solzen_quantile1231_desc(:,1:2),2)';

    pallavg.robs1  = ind54.rad_quantile_desc(:,:,9)';                  %% take avg
    pallavg.satzen = ind54.satzen_quantile1231_desc(:,9)';
    pallavg.solzen = ind54.solzen_quantile1231_desc(:,9)';
    ...
    for tt = 1 : length(thedir)
      prof = load(['../DATA_OneYear/' num2str(JOB,'%04d') '/pall_gridbox_' num2str(JOB,'%04d') '_16daytimestep_001_2013_01_01_to_2013_01_16.mat']);   %%% ERA PROFILE .. gives wierd solzen/satzen
      prof = load(['../DATA_OneYear/' num2str(JOB,'%04d') '/' thedir(tt).name]);                                                                      %%% ERA PROFILE .. gives wierd solzen/satzen

      prof.p2x = rmfield(prof.p2x,'satzen');
      prof.p2x = rmfield(prof.p2x,'scanang');
      prof.p2x = rmfield(prof.p2x,'zobs');
      ...
      profNclr.satzen(dadiffOK) = pall.satzen(dadiffOK);
      profNclr.zobs(dadiffOK)  = 705000;
      profNclr.solzen(dadiffOK) = pall.solzen(dadiffOK);
      profNclr.rtime(dadiffOK)  = pall.rtime(dadiffOK);
      ...
      profNcld.satzen(dadiffOK) = pall.satzen(dadiffOK);
      profNcld.zobs(dadiffOK)  = 705000;
      profNcld.solzen(dadiffOK) = pall.solzen(dadiffOK);
      profNcld.rtime(dadiffOK)  = pall.rtime(dadiffOK);
      ...
      profNavg.satzen(dadiffOK) = pallavg.satzen(dadiffOK);
      profNavg.zobs(dadiffOK)  = 705000;
      profNavg.solzen(dadiffOK) = pallavg.solzen(dadiffOK);
      profNavg.rtime(dadiffOK)  = pall.rtime(dadiffOK);
      ...
      profNcold.satzen(dadiffOK) = pallcold.satzen(dadiffOK);
      profNcold.zobs(dadiffOK)  = 705000;
      profNcold.solzen(dadiffOK) = pallcold.solzen(dadiffOK);
      profNcold.rtime(dadiffOK)  = pall.rtime(dadiffOK);
    ...
    iSave = +1;
    if iSave > 0
      rtpwrite_check(fnameclr,headN,[],profNclr,[]);
      rtpwrite_check(fnamecld,headN,[],profNcld,[]);
      rtpwrite_check(fnameavg,headN,[],profNavg,[]);
      rtpwrite_check(fnamecold,headN,[],profNcold,[]);
    end
