...
while iaChooseLatLonBin(1) > 0
  cder = ['cd  /umbc/xfs2/strow/asl/s1/sergio/MakeAvgObsStats2002_2020_startSept2002_CORRECT_LatLon/LatBin' num2str(iaChooseLatLonBin(2),'%02d') '/'];
  eval(cder)
  themall = load(['summarystats_LatBin' num2str(iaChooseLatLonBin(2),'%02d') '_LonBin_1_72_timesetps_001_412_V1.mat']);
  
  yy2002 = themall.yy(iaChooseLatLonBin(1),:) + (themall.mm(iaChooseLatLonBin(1),:)-1)/12 + (themall.dd(iaChooseLatLonBin(1),:)-1)/12/30;
  figure(1)
  plot(yy2002,themall.maxBT1231(iaChooseLatLonBin(1),:),'r',yy2002,themall.meanBT1231(iaChooseLatLonBin(1),:),'k',yy2002,themall.minBT1231(iaChooseLatLonBin(1),:),'b')                             %% ORIG
  plot(yy2002,themall.maxBT1231(iaChooseLatLonBin(1),:),'r',yy2002,rad2bt(1231,ttorad(1520,themall.meanBT1231(iaChooseLatLonBin(1),:))),'k',yy2002,themall.minBT1231(iaChooseLatLonBin(1),:),'b')   %% FIXED
  hl = legend('max','mean','min');
  xlim([min(yy2002) max(yy2002)]); title(['From all lonbins, choose bin ' num2str(iaChooseLatLonBin(1),'%03d')])
  
  %%%%%%%%%%%%%%%%%%%%%%%%%
  %% THIS HAS THE INDIVIDUAL SPECTRA
  cder = ['cd  LonBin' num2str(iaChooseLatLonBin(1),'%02d')]; eval(cder);
  ind54 = load(['summarystats_LatBin' num2str(iaChooseLatLonBin(2),'%02d') '_LonBin' num2str(iaChooseLatLonBin(1),'%02d') '_timesetps_001_412_V1.mat']);
  ...
  hl = legend('max','mean','min');
  xlim([min(yy2002) max(yy2002)]); title(['From LonBin' num2str(iaChooseLatLonBin(1),'%02d')])
  
  figure(3);
  %% ORIG
  plot(yy2002,themall.maxBT1231(iaChooseLatLonBin(1),:),'r',yy2002,themall.meanBT1231(iaChooseLatLonBin(1),:),'k',yy2002,themall.minBT1231(iaChooseLatLonBin(1),:),'b',...
  ...
  %% FIXED
  plot(yy2002,themall.maxBT1231(iaChooseLatLonBin(1),:),'r',yy2002,rad2bt(1231,ttorad(1520,themall.meanBT1231(iaChooseLatLonBin(1),:))),'k',yy2002,themall.minBT1231(iaChooseLatLonBin(1),:),'b',...
       yy2002,ind54.maxBT1231_desc,'m',yy2002,ind54.meanBT1231_desc,'g',yy2002,ind54.minBT1231_desc,'c')
  xlim([min(yy2002) max(yy2002)]); 
  ...
  indX = iaChooseLatLonBin(1); indY = iaChooseLatLonBin(2); %% iaChooseLatLonBin = input('Enter X/Y Lonbin/Latbin to look at (Lonbin 54 Latbin 32 -- is a good one, over Indonesia)  0 to stop: ');
  ...
  profNclr = do_clearcalc(headN,profNclr,[],1);
  profNcld = do_clearcalc(headN,profNcld,[],1);
  profNavg = do_clearcalc(headN,profNavg,[],1);
  profNcold = do_clearcalc(headN,profNcold,[],1);
  headN.pfields = 7;
  ...
  if iSave > 0
    fname = ['RTP_PROFS/Clr/timestep_lonbin_' num2str(indX,'%02d') '_latbin_' num2str(indY,'%02d') '_JOB_' num2str(JOB,'%04d') '_clr.rtp'];
    rtpwrite(fname,headN,[],profNclr,[]);

    fname = ['RTP_PROFS/Cld/timestep_lonbin_' num2str(indX,'%02d') '_latbin_' num2str(indY,'%02d') '_JOB_' num2str(JOB,'%04d') '_cld.rtp'];
    rtpwrite(fname,headN,[],profNcld,[]);

    fname = ['RTP_PROFS/Avg/timestep_lonbin_' num2str(indX,'%02d') '_latbin_' num2str(indY,'%02d') '_JOB_' num2str(JOB,'%04d') '_avg.rtp'];
    rtpwrite(fname,headN,[],profNavg,[]);

    fname = ['RTP_PROFS/Cold/timestep_lonbin_' num2str(indX,'%02d') '_latbin_' num2str(indY,'%02d') '_JOB_' num2str(JOB,'%04d') '_cold.rtp'];
    rtpwrite(fname,headN,[],profNcold,[]);
